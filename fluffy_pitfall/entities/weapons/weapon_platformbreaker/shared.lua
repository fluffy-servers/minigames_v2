		...
		if(dist < 100) then
			if SERVER then
				...
				if trace.Entity:Crouching() then
					trace.Entity:SetVelocity(trace.Entity:GetVelocity() + (self.Owner:GetAimVector() * 1200) )
				else
					trace.Entity:SetVelocity(trace.Entity:GetVelocity() + (self.Owner:GetAimVector() * 800) )
				end
