...
local have_killsound = CreateClientConVar("mg_killsound_enabled", 1, true, false)
...
function GM:AddDeathNotice2(attacker, inflictor, victim)
    ...
    elseif IsValid(attacker) and attacker:IsPlayer() then
        ...
        Death.color1 = table.Copy(NPC_Color)
        if IsValid(attacker) then Death.left = tostring(attacker) else Death.left = nil end
    end
    ...
    if IsValid(victim) and victim:IsPlayer() then
        ...
        if IsValid(victim) then Death.left = tostring(victim) else Death.right = nil end
    end
